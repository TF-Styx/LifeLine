...
namespace LifeLine.Employee.Service.Domain.ValueObjects.EmployeeType
{
    public sealed record EmployeeTypeName
    {
        ...
        private EmployeeTypeName(string value) { Value = value; }

        /// <exception cref="EmptyNameException"></exception>
        /// <exception cref="LengthException"></exception>
        public static EmployeeTypeName Create(string value)
        {
            GuardException.Against.That(string.IsNullOrEmpty(value), () => new EmptyNameException($"В структуру {nameof(EmployeeTypeName)} был передано пустое поле!"));
        ...
        public static implicit operator string(EmployeeTypeName name) => name.Value;
    }
}
