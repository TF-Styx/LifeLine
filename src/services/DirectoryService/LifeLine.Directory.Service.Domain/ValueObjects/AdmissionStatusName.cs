...
{
    public sealed record AdmissionStatusName
    {
        ...
        private AdmissionStatusName(string value) { Value = value; }

        /// <exception cref="EmptyNameException"></exception>
        /// <exception cref="LengthException"></exception>
        public static AdmissionStatusName Create(string value)
        {
            GuardException.Against.That(string.IsNullOrEmpty(value), () => new EmptyNameException($"В структуру {nameof(AdmissionStatusName)} был передано пустое поле!"));
            ...
            return new AdmissionStatusName(value);
        }

        public override string ToString() => Value.ToString();

        public static implicit operator string(AdmissionStatusName name) => name.Value;
    }
}
