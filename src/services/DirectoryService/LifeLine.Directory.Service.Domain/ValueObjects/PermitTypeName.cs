...
{
    public sealed record PermitTypeName
    {
        ...
        private PermitTypeName(string value) { Value = value; }

        /// <exception cref="EmptyNameException"></exception>
        /// <exception cref="LengthException"></exception>
        public static PermitTypeName Create(string value)
        {
            GuardException.Against.That(string.IsNullOrEmpty(value), () => new EmptyNameException($"В структуру {nameof(PermitTypeName)} был передано пустое поле!"));
            ...
            return new PermitTypeName(value);
        }

        public override string ToString() => Value.ToString();

        public static implicit operator string(PermitTypeName name) => name.Value;
    }
}
