...
namespace Shared.Serialization.JsonConverters
{
    ...
    {
        private static readonly Func<string, int, ErrorCode>? _createErrorCode = CreateConstructDelegate<string, int, ErrorCode>(BindingFlags.NonPublic | BindingFlags.Instance);

        private static Func<TArg1, TArg2, TResult>? CreateConstructDelegate<TArg1, TArg2, TResult>(BindingFlags flags) where TResult : struct
        {
            var ctor = typeof(TResult).GetConstructor(flags, null, [typeof(TArg1), typeof(TArg2)], null);
